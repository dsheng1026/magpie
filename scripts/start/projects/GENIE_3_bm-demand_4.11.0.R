...
cfg$input <- c(regional    = "rev4.119_5ff27be8_magpie.tgz",
              ...
              #  patch       = "SSP2_demand.tgz" # GHG price scaled to 2017USD
               patch       = "SSP2_demand_cap.tgz") # GHG price scaled to 2017USD and carbon price for non-co2 emissions are capped at 200 $(2017)/tC
...
cfg$gms$c56_pollutant_prices <- "G0000exp2110" # def = R34M410-SSP2-NPi2025
cfg$gms$c56_pollutant_prices_noselect <- "G0000exp2110" # def = R34M410-SSP2-NPi2025
...
beV <- c(0, 5, 7, 10, 15, 25, 45) # numbers need to be identicla to the input column name in the f60_bioenergy_dem.cs3 in patch_input
...
for (bl in blV) {
  ...
  for (mp in mpV) {
   ...
   cfg$info$flag2 <- preflag
    ...
    for (be in beV) {
      cfg$gms$c60_2ndgen_biodem <- paste0(preflag, "_BE", be)

      for (g in gV){
        g_str <- str_pad(g, 4, pad = "0")
        g_formatted <- paste0("G", g_str)
        cfg$gms$c56_pollutant_prices <- paste0(g_formatted, "exp2110")
        cfg$gms$c56_pollutant_prices_nonselect <- paste0(g_formatted, "exp2110")

        ##############################################
        cfg$title <- paste0(preflag, "_BE", str_pad(be, 2, pad = "0"), "_G", g_str, "demand")
