# |  Pack the inputs the price sweep reads.
# |
# |  The price sweep runs technological change exogenously (cfg$gms$tc = "exo"),
# |  which means it reads the land-use intensity trajectory out of an input file
# |  instead of solving for it. That file is f13_tau_scenario.csv and it comes
# |  out of the calibration run:
# |
# |      calibrate fulldata.gdx  ->  ov_tau (level)  ->  f13_tau_scenario.csv
# |                              ->  patch_input/<experiment>_price_<hash>.tgz
# |
...
# |
# |  The pipeline packs this itself, on the way from the calibrate phase to the
# |  price phase. Run it by hand to rebuild the packed inputs alone.
# |
# |  Usage
# |    Rscript messageix/R/pack_price.R [flags]     (from the model root)
# |
...
# |    --gdx=PATH           the calibration run's fulldata.gdx, when it is not
# |                         where the naming contract puts it
...
# |      pcfg <- with_patch(pcfg, 2, pack_price(pcfg))
# |
# |  A lever of the world whose mechanism is data contributes files here: every
# |  data lever registered for the price phase is asked what it wants packed, and
# |  its files travel in this tarball beside the trajectory. See
...
# |  through utils_runs.R, which carries the solvedness contract and the record
# |  of what the calibration run was solved under). Run from the MAgPIE model
# |  root. pack_demand.R sources this file for the shared helpers above
# |  (timestep_years, pack_patch) -- the packing contract is one contract, not two.
...
timestep_years <- function(timesteps, sets_file = "core/sets.gms") {
  ...
  pattern <- paste0("c_timesteps%\"\\s*==\\s*\"", timesteps, "\"")
  hits <- stringr::str_which(lines, pattern)
  hits <- hits[stringr::str_detect(lines[hits], stringr::fixed("/"))]
  if (!length(hits)) {
    log_die("timestep_years: no year list for c_timesteps '", timesteps, "' in ", sets_file)
  }
  # The year list is what sits between the two slashes on that line.
  years <- lines[hits[1L]] |>
    stringr::str_remove("^[^/]*/") |>
    stringr::str_remove("/[^/]*$") |>
    stringr::str_split_1(",") |>
...
# The c_timesteps token the packed files are validated against. It comes from
# the resolved experiment and nowhere else: the model horizon written into the
# working tree is rewritten by every run, so reading it from there would check
# the packed files against whichever run happened to be last.
...
pack_patch <- function(files, pcfg, stage) {
  ...
  name <- patch_tarball_name(pcfg$experiment, stage, files)
...
# has to change.
#   - a tau value of zero or below stops the price sweep outright: MAgPIE
...
validate_tau <- function(tau, years) {
  ...
  if (min(values) <= 0) {
    log_die("validate_tau: tau <= 0 (minimum ", signif(min(values), 4), ") in the calibration ",
            "export; modules/13_tc/exo/presolve.gms aborts the price sweep on this")
  }
...
# Pack the price sweep's inputs and return the tarball's name.
#
#   pcfg        resolved experiment
#   extra_dir   optional directory of additional override files (see --extra-files)
#   gdx         optional explicit calibration fulldata.gdx; by default the run
#               folder the naming contract assigns to the calibrate phase
pack_price <- function(pcfg, extra_dir = NULL, gdx = NULL) {
  assert_magpie_root()

  if (is.null(gdx)) {
    folder <- locate_run_folder(pcfg, 1L)
    if (is.na(folder)) {
      log_die("the calibration run folder was not found: ", run_folder(pcfg, 1L),
              ". Run the calibrate phase first, or pass --gdx=PATH")
    }
  ...
  if (!file.exists(gdx)) {
    log_die("the calibration run's ", RUN_GDX_FILE, " was not found: ", gdx,
            ". The run folder exists but holds no solved run")
  }
  log_step("EXTRACT", "tau from ", gdx)
  assert_run_solved(gdx, run_title(pcfg, 1L))

  # A calibration run folder is addressed by name and a re-run may overwrite it,
  # so the run itself records the settings it was solved under and they have to
  # be the ones this experiment asks for.
  ...
  files <- c(files, lever_patch_files(pcfg, 2L, stage_dir))
...
.synopsis_pack_price <- paste(
  "usage: Rscript messageix/R/pack_price.R [--experiment=NAME]",
  "[--set=key=value] [--gdx=PATH] [--extra-files=DIR] [--help]")

.usage_pack_price <- c(
  "Pack the land-use intensity trajectory (tau) of the calibration run into the inputs",
  "the price sweep reads. That sweep holds the trajectory fixed instead of solving for",
  "it, and reads it as f13_tau_scenario.csv out of",
  "<patch_repo>/<experiment>_price_<digest>.tgz.",
  "",
  "  Rscript messageix/R/pack_price.R [flags]   (from the MAgPIE model root)",
  "",
  "The pipeline packs this itself on the way from the calibrate phase to the price phase",
  ...
  "                      messageix/R/pipeline_infrastructure.R)",
  paste0("  --gdx=PATH          the calibration run's ", RUN_GDX_FILE,
         ", when it is not in the run folder the"),
  "                      naming contract points at",
  "  --extra-files=DIR   also pack every file in DIR alongside the trajectory. This is the",
  "                      way to carry a correction to some other input file; not needed",
  "                      when the base input tarballs are current. The directory must hold",
  "                      files only, no sub-directories.",
  "  --help              this text",
  "",
  "Every option is accepted as --key=value and as --key value.",
...
if (invoked_directly("pack_price.R")) {
  .opt <- parse_flags(commandArgs(trailingOnly = TRUE),
                      known      = c("experiment", "gdx", "extra-files"),
                      flags      = "help",
                      repeatable = "set",
                      usage      = .synopsis_pack_price)
  if (isTRUE(.opt$help)) {
    cat(.usage_pack_price, sep = "\n")
    cat("\n")
  } else {
    .pcfg <- config_from_flags(.opt, cli_overrides(.opt$set))
    log_step("CONFIG", "experiment '", .pcfg$experiment, "'")
    .name <- pack_price(.pcfg,
