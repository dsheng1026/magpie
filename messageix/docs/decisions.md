...
### Repository strategy — a clean branch in Di's fork — *Settled (18 Aug science review)*
...
The cost is a departure from the file format Di Sheng's team works in, and **that is Di's call,
not this build's**. Her team edits CSVs; the science, the naming and the golden runs are
identical either way, so what is at stake is only how the people who use the tool interact with
it. The question is open to her, and it is not either/or: if she prefers the CSV, a thin
importer that reads it and calls the same `experiment()` constructors sits on top of
`experiments.R` without changing anything below it, and both surfaces can exist at once. Asking
her which she wants is the next step, and this entry records the state of the question rather
than the end of it.
