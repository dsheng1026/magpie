...
**Every entry carries its status, because they are not the same age.** *Settled (18 Aug science
review)* marks a decision argued through with the people named in it. *New (19 Aug build)* marks
a decision this repository's build made on its own — a day old at the time of writing, and the
entries most worth challenging.

---

## Science decisions

### Q1 — Woodfuel double counting — *Settled (18 Aug science review)*
...
### Q2 — Non-CO2 carbon price cap — *Settled (18 Aug science review)*
...
independently in the sustainable-CDR paper. Earth Commission keeps the $200 cap.
...
### Q3 — The defaults table — *Settled (18 Aug science review)*

**Resolved: `c14_yields_scenario`, `c13_tccost`, `s30_annual_max_growth` and
`s44_cost_bii_missing` become user-configurable.**

All four were set from prior experiments and had been treated as fixed for the
...
### Q4 — Price grids — *Settled (18 Aug science review)*
...
### Q5 — Tau sensitivity — *Settled (18 Aug science review)*
...
### Q6 — Input tarball redistribution — *Settled (18 Aug science review)*

**Open, with a path.** Private within IIASA now. Escalation in order: the MAgPIE-side
collaborator talks to Keywan first, then the MAgPIE team (a meeting is scheduled to learn how
to update the tarball); team leaders settle public hosting later. Interim storage is a shared
drive belonging to the MAgPIE-side collaborator, with an IIASA-wide model-data share under
consideration. If the MAgPIE repository cannot host the
...
### Q7 — Undocumented tarball files and the missing steps — *Settled (18 Aug science review)*

**Resolved: `SSP2_tau.tgz` is dropped; two patch tarballs remain, both generated by code.**

`SSP2_tau.tgz` exists only because the base tarball the MAgPIE-side collaborator was handed is
stale relative to the MAgPIE version they run — it supplies files the newer version expects. With a correctly
...
### Q8 — Execution environment — *Settled (18 Aug science review)*
...
### Q9 — Upstream-file exceptions — *Settled (18 Aug fork investigation)*
...
### Repository strategy — a clean branch in the MAgPIE-side collaborator's fork — *Settled (18 Aug science review)*
...
work onto what may become an IIASA repository. The decision was for the clean branch, on
three grounds: the working branch is only about three commits ahead of the
...
### Deliberate deviations from the specification — *New (19 Aug build)*
...
### The configuration surface — three tiers — *New (19 Aug build)*
...
### The interface — experiment declaration plus one command — *New (19 Aug build)*
...
vocabulary is the four phases rather than the six internal steps.

The cost is a departure from the file format the MAgPIE-side collaborator's team works in, and
**that is the MAgPIE-side collaborator's call, not this build's**. Their team edits CSVs; the
science, the naming and the golden runs are identical either way, so what is at stake is only
how the people who use the tool interact with it. The question is open to the MAgPIE-side
collaborator, and it is not either/or: if they prefer the CSV, a thin importer that reads it
and calls the same `experiment()` constructors sits on top of `experiments.R` without changing
anything below it, and both surfaces can exist at once. Asking which they want is the next
step, and this entry records the state of the question rather than the end of it.

### The levers of the world are a registry — *New (19 Aug build)*
...
### Run names are ours — *New (19 Aug build)*
...
### One folder of code, one folder of data — *New (19 Aug build)*
...
### Table work is tidyverse — *New (19 Aug build)*
...
### Version pin — *Settled (18 Aug science review)*

The repository pins **v4.11.0**. The golden reference matrix is a v4.11.0 artefact, so this is
the version at which the golden-master test means anything. Upstream and the MAgPIE-side
collaborator's other working branch are at v4.14.0. Moving to 4.14.0 is the intended first version bump, which doubles as
the first exercise of the upstream-merge path.

---

## Divergences fixed rather than ported — *New (19 Aug build)*
...
deliberately on the move to 4.11.0 or is an unnoticed regression is one for the MAgPIE team.
...
- **The `f56_pollutant_prices.cs3` generator is missing.** Nothing in either source repository
  ...
  the MAgPIE team for the script. Failing that, read the twelve columns out of the existing
...
**Pending confirmation from the MAgPIE team:**
...
**To settle on the first golden build — nothing has been run yet:**

- **Intensive variables in the mapping.** All 81 mapping rows carry `spatial = reg+glo`, which
  asks for a global value beside the regional ones. That is right only where the global value
  comes from the run's own World row. Prices, `Biodiversity|BII` and `Landuse intensity
  indicator Tau` are per-unit quantities and must never be a sum across regions. The reduce
  phase tests it on every build — `check_intensive_aggregation()` compares each variable's World
  value against the sum over its regions and warns when the two are equal — but **no golden
  build has run, so the answer is not known**. On the first one, read the four CHECK lines the
  reduce phase prints for these variables and record the verdict here. If a World value comes
  out at roughly the regional total (around thirteen times a typical region at R12), that
  variable's mapping row changes to `spatial = reg`.
